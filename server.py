...
def create_label(max_classes):
    ...
    605: iPod
    761: remote control
    762: Restaurant / Eatery
    return single vector for now
    '''
    label[605] = 0.7;
    label[761] = 0.5;
    # for _ in range(random.randint(1, max_classes)):
    #     j = random.randint(0, vocab_size-1)
    #     label[j] = random.random()
    # label /= label.sum()
    return label

def create_labels(num, max_classes, singular=False):
    label = np.zeros((num, vocab_size))
    base_label = create_label(max_classes)
...
def create_random_images(num_images, max_classes):
    ...
    labels = create_labels(num_images, max_classes, singular=True)
...
def decode():
    try:
        ...
        vector = np.asarray([vector], dtype='float64')
        label  = np.asarray([label], dtype='float64')
        print('decode', vector, label)

        im = sample(vector, label, batch_size=1)

        return jsonify(encode_img(im[0]))
